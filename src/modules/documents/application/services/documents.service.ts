...
export class DocumentsService {
  ...
  async findAll(query: DocumentQueryDto): Promise<{
    data: (Document & { fileName: string; fileSize: number | null })[];
    total: number;
    page: number;
    limit: number;
    totalPages: number;
  ...
  async update(
    id: string,
    dto: Partial<CreateDocumentDto & { versionNote?: string }>,
    filePath?: string,
  ): Promise<Document & { fileName: string }> {
    const document = await this.documentRepository.findById(id);
  ...
  async delete(id: string): Promise<Document> {
    ...
    return {
      ...deletedDocument,
      status: deletedDocument.status as DocumentStatus,
    };
  }
}
