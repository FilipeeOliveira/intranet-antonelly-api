...
export class AuthController {
  ...
  @ApiOkResponse({
    description: "Link de recuperação enviado (se email existir)",
  })
  ...
  ): Promise<{ valid: boolean; message: string; expiresAt?: string }> {
  ...
  @ApiUnauthorizedResponse({ description: "Token inválido ou expirado" })
  @ApiBadRequestResponse({
    description: "Senhas não coincidem ou formato inválido",
  })
  ...
  @Throttle({ default: { limit: 3, ttl: 300000 } }) // 3 tentativas por 5 minutos
  @ApiBody({
    type: ChangeTemporaryPasswordDto,
    description: "Alterar senha temporária para permanente",
  })
  ...
  @ApiBadRequestResponse({
    description: "Senhas não coincidem ou usuário sem senha temporária",
  })
