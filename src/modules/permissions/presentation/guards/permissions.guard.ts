...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const hasAtLeastOne = requiredFeatures.some((f) => userFeatures.includes(f));

    // 🔹 Opção 2: precisa ter TODAS as features
    // const hasAll = requiredFeatures.every((f) => userFeatures.includes(f));
    if (!hasAtLeastOne) throw new ForbiddenException("Você não tem permissão para acessar este recurso/ação");
