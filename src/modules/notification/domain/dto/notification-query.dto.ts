...
export class NotificationQueryDto {
    ...
    @ApiProperty({ description: 'Campo para ordenação', enum: ['title', 'createdAt', 'updatedAt'], default: 'createdAt', required: false })
    @IsString()
    @IsOptional()
    sortBy: string = 'createdAt';

    @ApiProperty({ description: 'Filtrar por status de leitura', enum: ['read', 'unread'], required: false })
    @IsString()
    @IsOptional()
    read?: 'read' | 'unread';
