...
export class EmailService {
   ...
   */
  async sendCommunicationEmail(to: string[], context: CommuniqueEmailContext): Promise<void> {
    this.logger.log(`Enviando comunicado "${context.title}" para ${to.length} destinatários`);

    const severityInfo = SEVERITY_CONFIG[context.severity] || {
    ...
    this.logger.log(`Comunicado "${context.title}" enviado com sucesso`);
  }
}
